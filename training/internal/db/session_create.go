...
func CreateSession(session s.DBSession) (err error) {
	...
	query := `
		INSERT INTO ` + db.tableSessions + ` (
			...
			avr_speed_m_per_s, 
			avr_heart_rate_count_per_s, 
			avr_power_W,
			avr_vertical_oscillation_cm, 
			avr_ground_contact_time_ms, 
			avr_stride_length_m, 
			total_distance_m,
			total_steps_count, 
	...
	_, err = db.database.Exec(query,
		...
		session.AvrSpeedMPerS,
		session.AvrHeartRateCountPerS,
		session.AvrPowerW,
		session.AvrVerticalOscillationCm,
		session.AvrGroundContactTimeMs,
		session.AvrStrideLengthM,
		session.TotalDistanceM,
		session.TotalStepsCount,
		session.TotalEnergyBurnedKcal)
